...
Window {
    ...
    Timer {
        ...
        repeat: true
        onTriggered: {
            clockTime = Qt.formatTime(new Date(), "hh:mm")
    ...
    function parseColor(hex) {
        ...
        return Qt.rgba(parseInt(hex.substring(1,3), 16) / 255, parseInt(hex.substring(3,5), 16) / 255, parseInt(hex.substring(5,7), 16) / 255, 1)
    }
    ...
    function extIcon(name, isDir) {
        ...
        if (n.endsWith(".md") || n.endsWith(".txt")) return "\u{270E}"
        if (n.endsWith(".conf") || n.endsWith(".cfg")) return "\u2699"
        return "\u{1F4C4}"
    }
    ...
    function appIcon(name) {
        var n = name.toLowerCase()
        if (n.includes("terminal") || n.includes("alacritty") || n.includes("kitty")) return "\u{1F4BB}"
        if (n.includes("file") || n.includes("nautilus") || n.includes("dolphin")) return "\u{1F4C2}"
        if (n.includes("browser") || n.includes("firefox") || n.includes("chrome")) return "\u{1F310}"
        if (n.includes("editor") || n.includes("code") || n.includes("kate")) return "\u{270E}"
        ...
        if (n.includes("music") || n.includes("spotify")) return "\u{266B}"
        if (n.includes("video") || n.includes("vlc")) return "\u{25B6}"
        if (n.includes("steam") || n.includes("game")) return "\u{1F3AE}"
        if (n.includes("disc") || n.includes("chat")) return "\u{1F4AC}"
    ...
    Image {
        ...
        source: Data.wallpaper ? "file://" + Data.wallpaper : ""
        ...
        RowLayout {
            ...
            Rectangle {
                ...
                MouseArea {
                    ...
                    hoverEnabled: true
                    onClicked: {
                        root.showStartMenu = !root.showStartMenu
            ...
            Repeater {
                ...
                delegate: Rectangle {
                    ...
                    Text {
                        ...
                        color: index === root.activeWorkspace ? "#0c0c14" : "#6c7086"
                        font.pixelSize: 11
                        font.bold: index === root.activeWorkspace
                    }
                    MouseArea {
                        id: wsMA
                        anchors.fill: parent
                        hoverEnabled: true
                        onClicked: root.activeWorkspace = index
                    }
                }
            }
            ...
            Row {
                ...
                Repeater {
                    model: ["\u{1F50A}", "\u{1F4BB}", "\u{1F310}"]
                    delegate: Text {
                        text: modelData
                        font.pixelSize: 12
                        opacity: tMA.containsMouse ? 1.0 : 0.5
    ...
    GridView {
        ...
        cellHeight: 110
        z: 1
        model: Data.desktopFiles.length
        ...
        delegate: Item {
            width: 96
            height: 110
            property var fileData: Data.desktopFiles[index]
            ...
            Rectangle {
                anchors.fill: parent
                anchors.margins: 8
                color: dMA.containsMouse ? "#1e1e2e" : "transparent"
                radius: 12
                ...
                MouseArea {
                    ...
                    anchors.fill: parent
                    hoverEnabled: true
                    onDoubleClicked: console.log("Open:", fName)
                }
            }
        }
    }
    ...
    Rectangle {
        id: startMenu
        visible: root.showStartMenu
        width: 440
        height: 540
        x: 0
        y: root.height - panel.height - height - 8
        color: "#14141e"
        border.color: "#2a2a3c"
        border.width: 1
        radius: 16
        ...
        ColumnLayout {
            ...
            Rectangle {
                ...
                radius: 10
                color: "#1e1e2e"
                border.color: sI.activeFocus ? "#89b4fa" : "#2a2a3c"
                border.width: 1
            ...
            Row {
                spacing: 6
                Repeater {
                    model: ["All", "Frequent", "System", "Dev"]
                    delegate: Rectangle {
                        width: cT.width + 16
                        height: 24
                        radius: 12
                        color: index === 0 ? "#89b4fa" : "#1e1e2e"
                        border.color: "#2a2a3c"
            ...
            Rectangle {
                ...
                GridView {
                    ...
                    delegate: Item {
                        width: 88
                        height: 88
                        property var appData: Data.apps[index]

                        Rectangle {
                            anchors.fill: parent
                            anchors.margins: 6
                            color: aMA.containsMouse ? "#1e1e2e" : "transparent"
                            radius: 10
                            ...
                            MouseArea {
                                ...
                                anchors.fill: parent
                                hoverEnabled: true
                                onClicked: root.showStartMenu = false
                            }
                        }
                    }
                }
            ...
            Row {
                ...
                Rectangle {
                    ...
                    height: 34
                    radius: 8
                    color: pwrMA.containsMouse ? "#f38ba8" : "#1e1e2e"
                    border.color: "#2a2a3c"
                    border.width: 1
                    Behavior on color { ColorAnimation { duration: 150 } }
                    Text {
                        anchors.centerIn: parent
                        text: "\u23FB"
                        color: pwrMA.containsMouse ? "#0c0c14" : "#f38ba8"
                        font.pixelSize: 15
                    }
                    MouseArea {
                        id: pwrMA
        ...
        Rectangle {
            ...
            radius: 10
            z: 10
            opacity: root.showPowerMenu ? 1.0 : 0.0
            ...
            Column {
                ...
                Repeater {
                    model: [
                        { icon: "\u{1F512}", label: "Lock", clr: "#89b4fa", action: "lock" },
                        { icon: "\u{1F6AA}", label: "Log Off", clr: "#f9e2af", action: "logoff" },
                        { icon: "\u21BA", label: "Restart", clr: "#89b4fa", action: "restart" },
                        { icon: "\u23FB", label: "Power Off", clr: "#f38ba8", action: "poweroff" }
                    ]
                    ...
                    delegate: Rectangle {
                        ...
                        Row {
                            ...
                            Text {
                                text: modelData.icon
                                font.pixelSize: 11
                                color: piMA.containsMouse ? "#0c0c14" : "#6c7086"
                                anchors.verticalCenter: parent.verticalCenter
                            }
                            ...
                            Text {
                                ...
                                font.pixelSize: 11
                                color: piMA.containsMouse ? "#0c0c14" : "#cdd6f4"
                                anchors.verticalCenter: parent.verticalCenter
                            }
                        }
                        ...
                        MouseArea {
                            ...
                            onClicked: {
                                root.showPowerMenu = false
                                root.showStartMenu = false
                                // Call forge-power helper
                                var xhr = new XMLHttpRequest()
                                xhr.open("GET", "file:///usr/bin/forge-power")
                                // Use Qt.quit for logoff, system commands for others
                                if (modelData.action === "logoff") {
                                    Qt.quit()
    ...
    Shortcut {
        sequence: "Super"
        onActivated: root.showStartMenu = !root.showStartMenu
    }
